module robinson::shoreCurrency {
    ...
    // Declare an entry function `freeze_account` with parameters admin as signer, and account of type address and using `acquires` keyword access `MangedFungibleAsset`
    ...
    // using fungible_asset::set_frozen_flag(transfer_ref, wallet, true) to set the freeze account function of the account to true blocking all incoming or outgoing transfers. 
