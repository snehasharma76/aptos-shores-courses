module robinson::shoreCurrency {
    ...
    // Declare a function `deposit` with parameters admin as signer, to of type address, and fa to pass `FungibleAsset` by value and using `acquires` keyword access `MangedFungibleAsset`
    ...
    // using fungible_asset::deposit_with_ref(transfer_ref, to_wallet, fa) deposit the assets in the wallet.
