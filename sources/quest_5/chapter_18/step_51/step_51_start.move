module robinson::shoreNFT {
    ...
    // Define a public function `num_to_str` that takes a `u64` as input and returns a `vector<u8>`
    // Initialize an empty `vector<u8>` called `vec_data`
    // Declare a while loop, it will continue till the condition is true
    // Inside the loop push the last digit of `num` (obtained by `num % 10`) plus the ASCII value of '0' (48) to `vec_data`
    // Divide `num` by 10 to remove the last digit
    // If `num` becomes 0, exit the loop
    // Reverse the elements in `vec_data` to get the digits in the correct order
    // Return the `vec_data` vector, which now contains the byte representation of the string
