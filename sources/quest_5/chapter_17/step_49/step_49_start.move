module robinson::shoreNFT {
    ...
    public entry fun create_token(user: &signer) acquires CollectionCapability, MintInfo {
        ...
        // Generate a signer from the `constructor_ref` using `object::generate_signer` store the generated signer in the `token_signer` variable
        // Generate a mutator reference from the `constructor_ref` using `token::generate_mutator_ref` store the generated mutator reference in the `mutator_ref` variable
        // Generate a burn reference from the `constructor_ref` using `token::generate_burn_ref` store the generated burn reference in the `burn_ref` variable
        // Generate a transfer reference from the `constructor_ref` using `object::generate_transfer_ref` store the generated transfer reference in the `transfer_ref` variable
        // Update the `count` field of the `mint_info` resource with the `next_count` value
